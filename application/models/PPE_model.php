...
class PPE_model extends CI_Model {
    ...
    public function insert_ppe($data) {
        return $this->db->insert('inv_ppe', $data);
    }

    public function get_inventory_ppe($item_code = null, $date_from = null, $date_to = null) {
        $this->db->select('*');
    ...
    public function get_item_codes() {
        ...
        $this->db->from('inv_ppe'); // Adjust table name if needed
    ...
    public function update_balance($barcode, $new_balance) {
        $this->db->where('barcode', $barcode);
        $this->db->update('inv_ppe', ['balance' => $new_balance]);
    }
