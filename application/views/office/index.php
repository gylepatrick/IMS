...
<div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
    <h4><i class="fa-solid fa-boxes-stacked"></i> OFFICE SUPPLIES</h4>
    <div class="btn-group">
        <button type="button" class="btn btn-light shadow-lg btn-sm dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
            <b><i class="fa-solid fa-exchange-alt"></i> TRANSACTION</b>
        </button>
        <ul class="dropdown-menu">
            <li>
                <button class="dropdown-item" data-bs-toggle="modal" data-bs-target="#addOffice">
                    <i class="fa-solid fa-plus"></i> Add
                </button>
            </li>
            <li>
                <button class="dropdown-item" data-bs-toggle="modal" data-bs-target="#releaseOffice">
    ...
    <table id="example" class="table">
        <thead class="bg-primary text-white text-center">
            <tr>
                ...
                <th><i class="fa-solid fa-exchange-alt"></i> Transaction</th>
        ...
        <tbody>
            ...
            ?>
            <tr>
                <td><?php if($t->type != "Issuance" && $t->type != "Disposal") echo $t->purchased_date; ?></td>
                ...
                <td><?= $t->location ?></td>
                <td style="text-align: right;"><?= $t->unit ?></td>
                <td style="text-align: right;"><?= $t->quantity ?></td>
                <td style="text-align: right;"><?= number_format($t->unit_cost, 2) ?></td>
                <td style="text-align: right;"><?= number_format($t->total, 2) ?></td>
                <td>
                ...
                <button class="btn btn-warning btn-sm updateBtn"
                              ...
                              <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5z"/>
                                  ...
                                  <button class="btn btn-danger btn-sm deleteBtn" data-id="<?= $t->id ?>" data-type="<?= $t->type ?>" data-item_code="<?= $t->item_code ?>" data-batch="<?= $t->batch_number ?>" data-quantity="<?= $t->quantity ?>">
            ...
            <td ></td>
                <td ></td>
                <td><strong><span class="badge bg-danger text-white"><i class="fas fa-balance-scale"></i> Balance</span></strong></td>
                <td ></td>
                <td ></td>
                <td ></td>
                <td ></td>
                ...
                <td style="text-align: right;">
                    <?= isset($balances[$t->item_code][$t->batch_number]['quantity']) 
                        ? $balances[$t->item_code][$t->batch_number]['quantity'] 
                        : 0 ?>
                </td>
               
                <td style="text-align: right;"><?= number_format($balances[$t->item_code][$t->batch_number]['unit_cost'], 2) ?></td>
                <td style="text-align: right;"><?= number_format($balances[$t->item_code][$t->batch_number]['total'], 2) ?></td>
                <td ></td>
                <td ></td>
                <td></td>
